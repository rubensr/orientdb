 ...
 * @since 8/7/13
 */
public class OSingleValuePrefixBTreeBucket extends ODurablePage {
  ...
  OSingleValuePrefixBTreeBucket(OCacheEntry cacheEntry, boolean isLeaf, OBinarySerializer<String> keySerializer,
      OEncryption encryption, String bucketPrefix) {
    super(cacheEntry);
  ...
  @SuppressFBWarnings("EI_EXPOSE_REP2")
  public OSingleValuePrefixBTreeBucket(OCacheEntry cacheEntry, OBinarySerializer<String> keySerializer, OEncryption encryption) {
    super(cacheEntry);
    this.encryption = encryption;
  ...
  public void remove(final int entryIndex) {
    ...
    final int entrySize;
    if (isLeaf) {
      entrySize = keySize + getObjectSizeInDirectMemory(OCompactedLinkSerializer.INSTANCE, entryPosition + keySize)
  ...
  public SBTreeEntry getEntry(int entryIndex) {
    ...
    if (isLeaf) {
      ...
      ORID value = deserializeFromDirectMemory(OCompactedLinkSerializer.INSTANCE, entryPosition).getIdentity();

      return new SBTreeEntry(-1, -1, bucketPrefix + key, value);
    } else {
  ...
  public int[] getLeftRightLinks(int entryIndex) {
    assert !isLeaf;
  ...
  public ORID getValue(int entryIndex) {
    ...
    return deserializeFromDirectMemory(OCompactedLinkSerializer.INSTANCE, entryPosition + OByteSerializer.BYTE_SIZE).getIdentity();
  }
  ...
  byte[] getRawValue(final int entryIndex) {
    ...
    final int valueSize = getObjectSizeInDirectMemory(OCompactedLinkSerializer.INSTANCE, entryPosition);
  ...
  void addAllWithPrefix(List<SBTreeEntry> entries, String bucketPrefix) {
    final int bucketPrefixSize;
    ...
    for (int i = 0; i < entries.size(); i++) {
      final SBTreeEntry entry = entries.get(i);
      addEntry(i, entry, false);
    }
  }

  void shrinkWithPrefix(final int newSize, final String bucketPrefix) {
    List<SBTreeEntry> treeEntries = new ArrayList<>(newSize);
    ...
    int index = 0;
    for (SBTreeEntry entry : treeEntries) {
  ...
  public boolean addEntry(int index, SBTreeEntry treeEntry, boolean updateNeighbors) {
    ...
    if (isLeaf) {
      serializedValue = OCompactedLinkSerializer.INSTANCE.serializeNativeAsWhole(treeEntry.value);
      entrySize += serializedValue.length;
    } else {
  ...
  public static final class SBTreeEntry implements Comparable<SBTreeEntry> {
    ...
    public SBTreeEntry(int leftChild, int rightChild, String key, ORID value) {
    ...
    @Override
    public int compareTo(SBTreeEntry other) {
